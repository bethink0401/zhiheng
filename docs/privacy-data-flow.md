...
| 对应任务 | S14-01 |
| 状态 | S14-01 已验收；发布前仍需完成 S14-02～S14-10 的专项审计 |
...
| 今日感受与生活情境 | 本机 SwiftData 记录库 | 用户可在“今日状态 → 历史”修改或逐条删除 | 默认否 |
| 微计划、日程与完成/跳过结果 | 本机 CareKitStore | 用户可在微计划历史删除单个计划及其任务和结果 | 默认否 |
...
- 回答所需的聚合 HealthKit 事实：当前汇总、时间范围、数据质量、覆盖情况、个人基线，以及哪些指标当前不可用。
- 数据模式标识，用于区分真实与演示事实。

### 4.2 主动请求计划 AI 解读时还可能发送

- CareKit 完成率。
- 最多 7 条本次计划中由用户主动填写的受限反馈。
...
- 真实姓名、手机号、身份证、联系方式或精确地址。
- 生活事件备注、自定义名称和底层记录 ID。
- 与回答无关的完整本地数据库。
...
S14-01 的自动检查固定了数据流顺序、两条外发门禁、AI 字段边界、通知中性内容、PDF 临时文件与外部副本责任，以及权限和删除入口。App 内入口为“我的 → 隐私与安全 → 查看隐私说明与数据流”。
