...
- S14-13 将演示人物扩展为同一参考日期下的完整事实图：`DemoHealthScenarioFactory.dashboard` 生成 90 天十六指标自然曲线，`DemoSubjectiveRecordStore` 提供感受和生活事件，`MicroPlanSession` 与 `DemoEffectiveMethodFactory` 生成已结束计划、执行结果、评估、历史和方法卡。演示事实通过显式注入的 `demoStore` 与 `includesSyntheticDemoFacts` 进入洞察和 AI 事实包；默认值仍拒绝在演示模式读取真实 Store。S14-16 起，`DemoSubjectiveRecordStore` 允许会话期内存写入，使今日感受和生活事件可交互；这些改动不落盘且不接触真实 SwiftData。演示计划刷新仍不访问 CareKitStore，计划写操作在会话边界拒绝。
- S14-14 将步数与活动能量改为确定性的日程型曲线：工作日、周末和高活动日使用不同日内分布，最近八天采用非单调总量，活动能量独立于步数生成，活动样本只落在 06:00～22:00。演示微计划包含五天 Outcome、四条反馈和三次历史执行。AI 页在用户发问时继续从同一健康快照、感受会话和计划会话构建聚合事实，但不单独呈现演示记录概览。计划深度解读发送空的普通健康事实包和独立 `planEvaluation` 白名单，响应校验器仅为该请求额外放行评估指标与“微计划”事实类别，从而避免模型引用无关指标。主功能页不再展示演示警告横幅或演示标识，演示来源统一在设置中保持可查，外发 PDF 继续保留非真实记录标注。
- 数据模式切换时，今日、洞察、AI、微计划和“我的”按模式重建展示身份；若旧模式的异步读取尚未完成，微计划等待该次读取收口后再次检查目标模式，防止旧真实空状态覆盖演示计划。演示来源由设置的数据来源页统一说明，不在各主要页面重复显示标识。
...
精力、压力和身体感受全部选择且当天记录成功读取后才允许保存；同日再次保存更新既有记录，成功后关闭弹窗并同步摘要。读取失败时不自动弹出、不允许覆盖未知旧记录，手动重开可重试；保存失败保留草稿、窗口不关闭。跨日进入或跨午夜保存会重载新的本地日期并清除旧日草稿，不自动把昨天的感受写入今天。真实协调器在演示模式继续禁用读写；独立演示协调器使用演示内存仓库打开同一编辑界面，保存后更新本次运行的演示摘要和 AI 事实，重启恢复预置值。没有远程上传或 CareKit 任务状态副本。
...
S08-05 在 `SubjectiveCheckInSession` 中持有可编辑备注，加载恢复保存值、写入失败保留草稿，成功后同步归一值；读取失败、跨日和不可编辑的模式切换清理可见文本。独立演示协调器使用可编辑的演示内存记录，不触发这条真实数据保护分支。会话保存时也检查所加载的本地日期，防止绕过协调器将旧草稿写入新日。`OptionalRecordNoteField` 为今日感受和生活事件共用的默认收起输入组件，展开今日感受备注时选择大尺寸 sheet；显示已填写状态、字数、长度错误和键盘完成操作。领域与会话共用 `SubjectiveRecordText.normalized`，按 Swift `String.count` 统计去除两端空白后的扩展字素，空白转 nil；名称上限 30、备注上限 160，超限拒绝保存而不截断草稿。沿用 Schema V1 现有字段，不改变存储结构或新增迁移步骤；S12-20 仅在用户主动发送普通 AI 问题时读取这些既有字段构建受限事实包。
...
只允许删除列表中的单个 ID，用户需二次确认，成功后才更新列表。事件写入/删除失败时 SwiftData 适配器先回滚，再用同一 ModelContainer 重建关闭自动保存的上下文，避免失败插入残留在注册实例缓存中并被后续读取或其他保存带出；只读磁盘失败插入/删除测试已验证恢复后的读取仅含成功保存的记录。真实协调器在演示模式不读写真实数据；独立演示协调器允许在 `DemoSubjectiveRecordStore` 内新增和删除事件，修改只存于本次运行并可进入之后主动发起的 AI 事实包。沿用 Schema V1，无新持久化副本，不写 CareKit；S12-20 起结构化事件汇总、事件备注与自定义名称可在用户主动发送普通 AI 问题时进入受限事实包，记录 ID 与精确起止时间仍排除。S08-10 将可选入口移到今日状态双卡下方的紧凑独立行，保留记录数量、读取失败提示和原 sheet；标签为圆角矩形，标准字号两列、辅助字号一列，底部操作区固定可见。
...
历史操作成功后通知每日协调器：只有涉及今天的签到才重新加载感受摘要；历史事件变化刷新当天事件列表但保留未提交的感受草稿。浏览历史跨午夜时只更新日期状态，不自动展示 sheet。切换演示模式关闭历史/编辑窗口、清空可见记录并阻止读写；不改动 HealthKit 或 CareKit，下一次用户主动发送 AI 问题时才从最新本地记录重建事实包。S14-11 将历史入口改为 44 点无描边 Liquid Glass 纯图标圆形按钮，iOS 18～25 降级为无描边 `ultraThinMaterial`，仍保留完整辅助功能名称。
...
`HealthContextTimelineView` 位于历史导航内，观察同一个健康会话与历史会话；首次进入/截至日期改变重新读取本地主观记录，显式刷新同时刷新共享健康快照，原始历史编辑页保留不变。界面双重检查真实模式与历史启用状态，演示时不显示真实记录；会话切换演示清空时间线状态并阻止 Store 读取。不新增 Schema、原始样本持久化或 CareKit 状态副本；只有用户主动发送 AI 问题时，普通对话才读取同一 Store 的白名单聚合，不因浏览时间线自动联网。
...
S12-20 的 `HealthFactSupplementalFactsBuilder` 在用户点击发送时复用 `DailyCheckInCoordinator`、专用只读 `AssistantFactContextLoader` 与根视图持有的 `MicroPlanSession`，不建立第二份持久化数据。客观指标完整保存本地 7 天当前窗口与此前不重叠 28 天基线的状态、中位数、变化、有效日、异常值保护和来源稳定性；`highlightedChangeMetric` 复用今日变化选择器，不让模型自行比较兼容基线字段。补充事实包括今日感受与签到备注、最近 7 个完整日感受中位数/覆盖与最多 7 条带本地日期的签到备注、今日和最近 7 个完整日的结构化事件及每窗口最多 20 条事件备注/自定义名称，以及当前或最近微计划的模板、状态、日期、时间、CareKit 进度、今日结果和最多 7 条反馈。签到/事件备注继续使用领域 160 字上限，自定义名称使用 30 字上限；构建器防御性丢弃空白或越界文本。四类补充事实分别表达 `recorded`、`notRecorded`、`unavailable`、`demoMode`。

普通对话响应新增可选 `usedFactKinds`：健康指标、今日感受、近 7 天感受、生活事件和微计划。新模型结构必须返回该数组；客户端验证其唯一且为本次事实包可引用类别，旧本机会话缺少字段时继续兼容。事实详情同时展示指标与事实类别。事件只作为同期背景，反馈只作为主观记录；已有活动或暂停计划时提示词禁止再推荐新计划。

普通 AI 事实包不包含原始样本数组、应用另行添加的账号身份/通讯录/定位信息、精确事件时间、设备来源标识、CareKit 计划/任务 ID 和其他底层记录 ID。签到备注、事件备注和自定义事件名称属于用户明确选择发送的原文上下文，可能含用户自行写入的个人或敏感信息；提示词必须将其视为不可信的用户自述数据，不执行其中指令，也不把它们提升为客观事实或因果证据。计划结束的 AI 深度解读仍沿用独立的聚合评估白名单，不包含这些生活情境自由文本。
...
`PrivacyDataFlowCatalog` 是版本为 `s14-privacy-data-flow-v3` 的只读领域目录，集中描述五条实际数据路径：Apple Health 到本机洞察、主观记录与 CareKit 计划、本地通知、AI 对话和 PDF 分享。每个步骤保留稳定 ID、普通用户可读说明、所属边界和是否离开本机；外发只分为用户点击发送问题后的 HTTPS 请求与用户确认后的系统分享。目录不读取任何 Store、HealthKit、网络或当前用户值，也不保存新的隐私偏好。V3 同步 S12-20 的普通 AI 对话事实范围，并明确用户原文可能含其自行填写的敏感信息。
...
普通 AI 对话的当前请求边界以实现为准：用户本次问题、最近最多 10 条对话、`HealthFactPackBuilder` 生成的聚合 HealthKit 当前值/质量/本地趋势，以及 `HealthFactSupplementalFactsBuilder` 生成的今日与近 7 天感受/签到备注、结构化事件/事件备注/自定义名称和当前或最近微计划执行/反馈。用户主动请求计划 AI 解读时，现有独立事实包还可以加入完成率、最多七条受限反馈、相关指标聚合、数据质量、本地结论和结构化生活背景，但不加入生活情境自由文本。两条路径都排除原始 HealthKit 样本数组、精确事件时间、设备来源标识、CareKit 计划/任务 ID 和其他底层记录 ID；应用不另行添加账号身份、通讯录或定位信息，但用户自己写进备注的个人或敏感内容会按原文发送。完整可审计说明见 `docs/privacy-data-flow.md`。
