...
- 今日页展示连续 7 日选择、五项目标完成度、当日身体/日常数据和昨晚生命体征；身体区在 HRV 与训练准备两卡下方只允许额外显示一项由本地确定性规则选出的“今日变化”摘要，完整趋势与解释仍由指标详情和洞悉页承担。顶部专属观察图标、“发生了什么”和“建议”三个图标统一采用青绿色图形与青绿色透明圆底。
...
当前 `MicroPlanEvaluationFactory` 先基于 CareKit 完成率、Outcome 反馈、任务相关 HealthKit 计划前后趋势和数据质量生成确定性本地结果。完成率低于 60% 时优先输出“执行不足”；计划前或计划期有效日少于 2 天时不计算对应客观变化；所有结果只使用批准的非因果措辞。结束卡片展示完成率、反馈天数、客观趋势和数据质量，删除重复的解释性说明。

“AI 解读”是用户主动触发的可选层。确认后，`HealthAIRequest.planEvaluation` 只携带完成率、最多七条受限反馈、相关指标聚合、数据质量和本地结论；不包含原始 HealthKit 样本。客户端校验 AI 不新增计划、不引用事实包外指标、不输出诊断或因果结论；失败时继续显示本地评估。
